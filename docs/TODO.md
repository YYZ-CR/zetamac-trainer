...
## 0c. Division asked questions Zetamac never asks — fixed and applied

Both generators built a division as a reversed product and then **flipped a coin**
over which factor became the divisor: `(a×b) ÷ a` or `(a×b) ÷ b`. Zetamac never
flips. The divisor is always the first multiplication range, so under the default
2–12 × 2–100 the real game asks `876 ÷ 12` and ours also asked `876 ÷ 73` — about
42% of divisions, measured. Dividing by an arbitrary two-digit number is a different
skill from dividing by a times-table number, so every score set under it was against
a harder game than the one it was compared to. Reported by a player, then confirmed
against the official site.

Fixed in `js/game.js` (solo) and `daily_generate_questions` in `supabase/daily.sql`,
which is the only server-side generator — `duels.sql` and `steal.sql` both call it.
Subtraction still flips deliberately: it is a reversed addition, its two ranges are
identical under the default, so which operand is subtracted is not observable.

- [x] **`supabase/daily.sql` has been re-applied** against the real project, so the
      daily, duel and steal generators no longer flip. Solo play is fixed by the
      static deploy alone.
- [ ] **Today's daily puzzle is already stored** with flipped questions and is not
      regenerated by re-applying the file. It ages out at the next UTC midnight;
      deleting today's `daily_puzzles` row would invalidate attempts already in
      progress, so the cheap move is to wait rather than to fix it.
- Covered by `test_game.js` (client, in `npm test`) and three new assertions in
  `supabase/test/03-daily-test.sql` (server). `test_game.js` was run against the
  pre-fix `game.js` and fails on it — 1275 of 3000 divisors landed outside 2–12 —
  so it is known to catch the bug rather than merely to pass. **The SQL assertions
  have not been executed**: the machine they were written on has no Postgres. CI
  runs them on the next push.
